...
import { login, signup, resetPassword } from '@/app/actions';
import { useSearchParams } from 'next/navigation';

import { useFormStatus } from 'react-dom';

function SubmitButton({ isLogin, isReset }: { isLogin: boolean; isReset: boolean }) {
  ...
  return (
    ...
    >
      ...
      {pending 
        ? (isReset ? 'Enviando...' : isLogin ? 'Entrando...' : 'Registrando...') 
        : (isReset ? 'Enviar Instrucciones' : isLogin ? 'Entrar a la Cancha' : 'Registrarme')}
    </button>
  );
}
  ...
  return (
    <div className="w-full max-w-md mx-auto relative z-10">
      ...
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-3xl shadow-2xl">
        ...
        <form className="space-y-5" action={isReset ? resetPassword : (isLogin ? login : signup)}>
          {!isLogin && !isReset && (
            <div>
          ...
          <div>
            ...
            <p className="text-xs text-[#CFB53B] mb-2 font-medium">
              Por favor ingresa el correo con el que te inscribiste
            </p>
            <input
              id="email"
              name="email"
              ...
              {isLogin && (
                <div className="mt-2 text-right">
                  <button
                    ...
                    className="text-xs text-gray-400 hover:text-[#CFB53B] transition-colors"
                  >
          ...
          <SubmitButton isLogin={isLogin} isReset={isReset} />
        </form>
          ...
          <p className="text-gray-400 text-sm">
            {isReset ? (
              <button
                ...
                className="font-bold text-[#CFB53B] hover:text-white transition-colors"
              >
                Volver al Inicio
              </button>
            ) : (
